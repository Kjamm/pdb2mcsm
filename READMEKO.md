...
MD 시뮬레이션 출력이나 그 밖의 raw 구조 파일은 mCSM-PPI2에 그대로 못 넣는 경우가 많습니다. 사슬이 합쳐져 있거나 chain 표시가 없고, 잔기 번호가 1부터 다시 매겨져 있고, 물·이온이 잔뜩 붙어 있고, 히스티딘이 `HIE` 같은 MD 전용 이름으로 적혀 있기 때문입니다. `pdb2mcsm`은 이런 문제를 자동으로 고쳐서 서버가 받아들이는 형태로 만들어줍니다.
